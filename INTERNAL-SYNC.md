...
| 2026-07-16 | **Orb rebuilt + repositioned**: now the shared component doshus.net uses sitewide — spinning whorl avatar in a darker-purple core, docked BELOW the Doshus.NET button (was above; its tip collided with the strip buttons' upward tooltips), tooltip points down, quip pool 6→19; markup switched from `<a class="zephyy-orb">` to the `zephyy-orb-sitewide-wrapper printmon-dock` block; external busters now `swapbtn.css?v=orb6` / `swap-img.js?v=orb7` (external-only as always). External pages also reference `../../css/zephyy-orb-embed.css` — internal does NOT mirror that file; the needed rules are inlined into the swapbtn.css payload | all strip pages, `css/swapbtn.css` (old orb block replaced), `js/swap-img.js` (orb IIFE replaced: quips + whorl injection + below-positioning) | **INTERNAL-SYNC-PROMPT.md fully rewritten 2026-07-16** — use as-is in Quick Suite; payloads are self-contained (no external css file needed internally). Classics still skip the orb internally |
